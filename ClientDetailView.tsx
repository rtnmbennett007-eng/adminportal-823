        ...
        <div className="bg-[#0b1528] border border-blue-900/60 p-6 rounded-2xl shadow-xl space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-blue-900/60 pb-4">
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-slate-400">
                  Decision: <strong className="text-amber-300">{underwritingDecision}</strong>
                </span>
              </div>
              <h2 className="text-lg font-bold text-slate-100 mt-1">
                Risk Analysis & Submission Authorization
              </h2>
            </div>
            ...
            >
              ...
              <span>Save Underwriting File</span>
            </button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Underwriter Decision</label>
              <select
                value={underwritingDecision}
                onChange={(e) => setUnderwritingDecision(e.target.value as any)}
              ...
              >
                <option value="QUALIFIED">QUALIFIED - Tier 1</option>
                <option value="PRE_APPROVED">PRE-APPROVED</option>
                <option value="APPROVED">APPROVED FOR CAPITAL</option>
                <option value="ADDITIONAL_INFO_REQUESTED">ADDITIONAL INFO REQUESTED</option>
                <option value="NOT_QUALIFIED">NOT QUALIFIED / DECLINED</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Recommended Amount</label>
              <input
                type="number"
                value={recommendedAmount}
                onChange={(e) => setRecommendedAmount(parseFloat(e.target.value) || 0)}
                className="w-full bg-[#070d18] border border-blue-900/70 rounded-xl p-2.5 text-xs text-emerald-300 font-mono font-bold focus:outline-none"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setRecommendedProduct(e.target.value as any)}
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3">
              Underwriting Verification Matrix
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {Object.entries(underwritingChecklist).map(([item, status]) => (
                <div
                  key={item}
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setUnderwritingChecklist({
                        ...
                        [item]: e.target.value as any,
                      })
                    }
                    className="bg-[#0b1528] border border-blue-800 rounded px-2 py-1 text-xs text-slate-200"
                  >
          ...
          {/* Underwriting Employment & Compensation Assessment */}
          {masterVerification?.employmentVerification && (
            <div className="p-4 rounded-xl bg-[#070d18] border border-blue-900/50 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-amber-400 uppercase tracking-wider flex items-center gap-2">
                  <Briefcase className="w-4 h-4" />
                  Verified Employment & Capacity Underwriting
                </h3>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 font-bold">
                  Status: {masterVerification.employmentVerification.sectionStatus || 'Verified'}
                </span>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                <div>
                  <span className="text-[10px] text-slate-400 block">Employer & Title:</span>
                  <span className="font-semibold text-slate-200">
                    {masterVerification.employmentVerification.employerName?.verified || 'Apex Healthcare'} (
                    {masterVerification.employmentVerification.jobTitle?.verified || 'Director'})
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block">Verified Annual Salary:</span>
                  <span className="font-bold text-emerald-400 font-mono">
                    {masterVerification.employmentVerification.annualSalary?.verified || '$145,000'}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block">Payroll & Frequency:</span>
                  <span className="font-semibold text-cyan-300">
                    {masterVerification.employmentVerification.paidThroughPayroll?.verified === 'Yes' ? 'ADP / Payroll' : 'Direct'} (
                    {masterVerification.employmentVerification.payFrequency?.verified || 'Biweekly'})
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 block">Pay Stubs Reviewed:</span>
                  <span className="font-bold text-emerald-400 font-mono">
                    {masterVerification.employmentVerification.payStubReviewed?.verified || 'Yes'} (
                    {masterVerification.employmentVerification.mostRecentPayStubDate?.verified || 'Current'})
                  </span>
                </div>
              </div>

              {masterVerification.employmentVerification.redFlags && masterVerification.employmentVerification.redFlags !== 'None' && (
                <div className="p-2.5 rounded-lg bg-rose-950/30 border border-rose-900/50 text-xs text-rose-300 flex items-start gap-2">
                  <AlertTriangle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />
                  <div>
                    <strong className="block text-[10px] uppercase font-bold text-rose-400">Employment Red Flags:</strong>
                    {masterVerification.employmentVerification.redFlags}
                  </div>
                </div>
              )}
            </div>
      ...
      {isEditingClient && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0c1832] border border-blue-800 rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-fadeIn">
            <div className="p-4 bg-[#081124] border-b border-blue-900 flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-100">Edit Client Master File</h3>
              <button
            ...
            <form onSubmit={handleSaveMasterClient} className="p-5 overflow-y-auto space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editClientForm.federalTaxId || ''}
                    onChange={(e) => setEditClientForm({ ...editClientForm, federalTaxId: e.target.value })}
                    className="w-full bg-[#070d18] border border-blue-900/70 rounded-xl p-2.5 text-slate-100 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Monthly Revenue</label>
                  <input
                    type="number"
                    value={editClientForm.monthlyRevenue || 0}
                    onChange={(e) => setEditClientForm({ ...editClientForm, monthlyRevenue: parseFloat(e.target.value) || 0 })}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={editClientForm.annualRevenue || 0}
                    onChange={(e) => setEditClientForm({ ...editClientForm, annualRevenue: parseFloat(e.target.value) || 0 })}
                    className="w-full bg-[#070d18] border border-blue-900/70 rounded-xl p-2.5 text-slate-100 focus:outline-none font-mono"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={editClientForm.creditScore || 700}
                    onChange={(e) => setEditClientForm({ ...editClientForm, creditScore: parseInt(e.target.value) || 0 })}
                    className="w-full bg-[#070d18] border border-blue-900/70 rounded-xl p-2.5 text-slate-100 focus:outline-none font-mono"
                  />
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-blue-900 flex justify-end space-x-2">
                ...
                >
                  {isSavingMaster ? 'Saving...' : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>

    ...
    <header className="h-16 bg-[#0b1528] border-b border-blue-900/40 px-6 flex items-center justify-between sticky top-0 z-40 shadow-md">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div ref={userMenuRef} className="relative pl-2 border-l border-blue-900/60">
            ...
            <div className="hidden md:block text-left">
              <div className="text-xs font-bold text-slate-100 leading-tight flex items-center gap-1.5">
                ...
                <span className="text-[9px] px-1.5 py-0.2 rounded bg-amber-500/10 text-amber-300 border border-amber-500/30 font-mono">
                  {currentUser?.role === 'INTERNAL_STAFF_ADMIN' ? 'ADMIN' : 'STAFF'}
                </span>
              </div>
              <div className="text-[10px] text-blue-300/80 truncate max-w-[120px] leading-tight">
                {currentUser?.jobTitle || 'Director of Operations'}
              </div>
            </div>
          ...
          {/* User Popover Menu */}
          {showUserMenu && (
            <div className="absolute right-0 top-full mt-2 w-64 bg-[#0e1c38] border border-blue-800 rounded-2xl shadow-2xl z-50 overflow-hidden animate-fadeIn">
              <div className="p-3.5 bg-[#091222] border-b border-blue-900/80">
                <div className="text-xs font-bold text-slate-100">{currentUser?.name}</div>
                <div className="text-[11px] text-slate-400">{currentUser?.email}</div>
                <div className="mt-1 flex items-center gap-1.5">
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/30 font-semibold font-mono">
                    {currentUser?.role || 'INTERNAL_STAFF_ADMIN'}
                  </span>

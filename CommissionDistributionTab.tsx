...
export const CommissionDistributionTab: React.FC<CommissionDistributionTabProps> = ({
  ...
  const handleDeleteParticipant = async (participantId: string, name: string) => {
    if (!confirm(`Are you sure you want to remove ${name} from this commission split?`)) return;

    try {
      await deleteCommissionParticipant(participantId);
      addToast('success', 'Participant Removed', `${name} has been removed from commission distribution.`);
      onRefresh();
    } catch (err: any) {

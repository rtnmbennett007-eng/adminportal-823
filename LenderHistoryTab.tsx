...
export const LenderHistoryTab: React.FC<LenderHistoryTabProps> = ({
  ...
  const handleDeleteRecord = async (id: string, lenderName: string) => {
    if (!confirm(`Are you sure you want to remove the submission record for ${lenderName}?`)) return;

    try {
      await api.deleteLenderHistoryRecord(id);
      addToast('success', 'Record Deleted', `Submission to ${lenderName} removed.`);
      onRefresh();
    } catch (err: any) {

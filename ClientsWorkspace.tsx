...
export const ClientsWorkspace: React.FC<ClientsWorkspaceProps> = ({
  ...
  const handleDeleteClient = async (c: Client, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm(`Are you sure you want to delete client file for ${c.firstName} ${c.lastName} (${c.businessName})? This will delete all associated deals, verifications, and underwriting records.`)) {
